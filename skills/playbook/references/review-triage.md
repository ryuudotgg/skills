...
Use this reference when the Babysit playbook (`../playbooks/babysit.md`) handles comments from a review bot or any other review automation. The goal is not to ignore the bot by default. The goal is to stop treating every comment as a required code change.
...
- Confidence: candidate
- Skip when: The bot flags an export, component, helper, or file as unused, and upper-stack diffs or PR context show it is used by a later PR in the stack.
...
- Skip when: The comment body or a later reply from the bot explicitly says the finding is withdrawn, compliant, or a false positive, and the agent can verify the relevant rule locally.
...
- Source: one sticky-occlusion PR, six review passes, every finding fixed rather than dismissed.
...
- Source: one prose-pinning PR with eight review passes; the claim was real on
  a late pass despite every earlier pass being fixed-and-resolved.
