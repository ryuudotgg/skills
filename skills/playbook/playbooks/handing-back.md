...
Read `../references/delivery.md` before step 7. Babysit takes its own branch below, and under `/plans do` the plans skill picks up after the publish.
...
7. Run `../scripts/delivery-mode.sh` with `sh` and quote its output.
   - **hands-off**, a babysit that ran hands-off, or you are a delegate: stop. Do not stage, do not commit, do not push, do not open a PR. The work stays unstaged on the `feat/*` branch in the main tree, where the operator picks it up.
