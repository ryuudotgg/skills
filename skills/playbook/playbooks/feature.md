...
1. `how` over the affected subsystem. For a browser-rendered surface, also read `../references/ui-design.md` and record the surface's mode, the incumbent visual system, and what must survive the change.
...
5. Verify on the matching surface. "Inconclusive" or wrong-surface is not a pass; flag it. For UI, run the Verify list in `../references/ui-design.md` through the live browser MCP in one batched round, desktop and mobile together, fix in one batch, confirm once. Record the route, viewports, and states tested.
