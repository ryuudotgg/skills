...
1. Route through the **how** skill (Explain mode for narrow questions, Critique mode for "are we sure?"). A motivation question ("why is it shaped this way") is answered from the history: `git log -S<symbol>`, `git log -p` over the file, and the PR discussion read with `gh pr view`. Reading the remote is fine, posting to it is not.
