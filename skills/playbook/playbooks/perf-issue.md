...
3. Plan the fix from the trace. If it crosses a function boundary, `architect` first. Delegate implementation to the `codex-astra` wrapper agent, which shells out to the Codex CLI (`codex exec --enable fast_mode -m gpt-6-astra -c model_reasoning_effort=high -C <abs repo path> -o /tmp/codex/<slug>.md`); review the diff. Never pass an `isolation` parameter; worktrees are banned. Capture a post-fix trace.
