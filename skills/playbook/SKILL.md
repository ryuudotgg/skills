...
- Before you hand work back → the **no-comments** skill (`/no-comments`), then the three-line style check: no comments that restate the code, no em dashes or en dashes or hyphen-as-dash, no AI-sounding copy.
- Shipping UI, or any browser-rendered surface → drive it live. A browser-driving MCP for a real browser (Playwright or equivalent, tool names matching `mcp__*playwright*`), or whatever preview MCP the harness exposes. For bug fixes, reproduce first on the same surface yourself; hand to the user only under the narrow Bug fix step 1 exception.
