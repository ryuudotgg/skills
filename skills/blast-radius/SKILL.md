...
6. For a big or wide change, ask the same question of several models and merge the answers. Two Claude arms (`fable-max`, `opus-xhigh`) and two Codex arms (`codex-astra`, `codex-terra`), all read only. Different models catch different real bugs. Each name is a wrapper agent that owns its own model choice. Skip any arm whose wrapper is not installed and run the rest. Pass no `model` parameter and no `isolation` parameter in any form; every value of it produces a worktree, and worktrees are banned. Scratch work goes in a directory under `/tmp/`.
