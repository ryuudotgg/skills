...
6. For a big or wide change, ask the same question of several models and merge the answers. Two Claude arms (`fable-judgment`, `opus-xhigh`) as subagents and two Codex arms (astra, terra) as background Bash calls per the playbook skill's **Codex arms** section, all read only. Different models catch different real bugs. With no `codex` on PATH run the Claude arms and say so; skip any Claude agent that is not installed and run the rest. Pass no `model` parameter and no `isolation` parameter in any form; every value of it produces a worktree, and worktrees are banned. Scratch work goes in a directory under `/tmp/`.
