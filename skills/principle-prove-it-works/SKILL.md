...
Keep the artifact visible for the human: name its path in the handover. A check script can be one of the task's files, so it is delivered with them, per the playbook skill's `references/delivery.md`. For a large port or migration where the trail has to stay auditable, say so in the handover so the human can decide what to keep (the **show-me-your-work** skill).
