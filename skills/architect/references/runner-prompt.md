...
- Make boundaries visible. `not implemented` errors for bodies, and pseudocode for tricky logic written as the body, not as a comment. A reader should trace data from input to output by reading types and signatures alone. The sketch carries no comments: if intent or an invariant is not readable from the names and types, fix the names and types.
