...
PLANS_DO = re.compile(r"/plans\s+do\b|plans</command-name>\s*<command-args>\s*do\b")
...
def transcript_event(record: dict[str, Any]) -> TranscriptEvent | None:
    ...
    return TranscriptEvent(
        timestamp, event_type, extracted_text(content), tuple(blocks), model, usage, bool(record.get("isMeta"))
    )
...
def read_windows(cutoff: datetime) -> tuple[list[Window], list[Subagent]]:
    ...
    for path in recent_files(main_pattern, root, cutoff, "windows"):
        events = [event for record in records_from_file(path, "windows") if (event := transcript_event(record))]
        windows.extend(w for w in windows_from_events(events) if w.events[0].timestamp >= cutoff)
...
def read_codex_runs(cutoff: datetime) -> list[CodexRun]:
    ...
    for path in recent_files(pattern, root, cutoff, "codex"):
        ...
        runs.append(
            CodexRun(setting(models), setting(efforts), min(timestamps), max(timestamps), originator)
        )

    if mixed:
        note("codex", f"{mixed} rollout(s) changed model or effort mid run and are grouped as mixed")

    return runs


def summarize_tasks(tasks: list[Task]) -> dict[str, Any]:
    durations = sorted((task.ended_at - task.started_at).total_seconds() / 60 for task in tasks)
...
def window_metrics(window: Window) -> dict[str, float | None]:
    ...
    model_s = 0.0
    subagent_wait_s = 0.0
    tool_intervals: defaultdict[str, list[tuple[datetime, datetime]]] = defaultdict(list)
...
def summarize_windows(windows: list[Window], subagents: list[Subagent]) -> dict[str, Any]:
    ...
    tails = sorted(float(metric["tail_s"]) for metric in kept if metric["tail_s"] is not None)
    ...
    for agent_type, values in subagents_by_type.items():
        ...
        subagent_breakdown.append(
            {"agent_type": agent_type, "n": len(values), "median_wall_minutes": median}
        )

    subagent_breakdown.sort(key=lambda row: (-int(row["n"]), str(row["agent_type"])))
...
def summarize_codex_runs(runs: list[CodexRun]) -> dict[str, Any]:
    ...
    for run in runs:
        wall_minutes = max(0.0, (run.ended_at - run.started_at).total_seconds() / 60)
    ...
    summary_groups.sort(key=lambda row: (-int(row["n"]), str(row["model"]), str(row["effort"])))
    return {"n": len(runs), "resumed_excluded": resumed, "groups": summary_groups}
...
def render(result: dict[str, Any]) -> str:
    ...
    codex_rows = [
        (row["model"], row["effort"], row["n"], row["median_wall_minutes"], row["p75_wall_minutes"])
    ...
    parts.extend(
        [
            ...
            "Subagents",
            text_table(("n", "total wall hours"), [(subagents["n"], subagents["total_wall_hours"])]),
    ...
    parts.extend(
        [
            ...
            text_table(("model", "effort", "n", "median wall minutes", "p75 wall minutes"), codex_rows),
        ]
    )
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Report task timing from local session stores.")
    ...
    plans_dir = Path(os.environ.get("PLANS_DIR", str(Path.home() / "Plans"))).expanduser()
