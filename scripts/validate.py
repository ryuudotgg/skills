...
def check_frontmatter(path, expected_name):
    ...
    if data.get("name") != expected_name:
        err(path, 1, f"name is {data.get('name')!r}, directory says {expected_name!r}")
    for key in ("mode", "icon", "color", "reminder"):
        if key in data:
            err(path, 1, f"frontmatter key {key} is a Cursor chat-mode key, unsupported here")
    ...
    for n, line in enumerate(text.splitlines(), 1):
        for ref in PATH_REF.findall(line):
            ...
            if any(c in ref for c in "<>*$ "):
                continue
            bases = (os.path.dirname(path), skill_root(path), ROOT, os.path.join(ROOT, "skills", "playbook"))
            first = ref.split("/")[0]
