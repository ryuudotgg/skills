...
LICENSE = re.compile(r"SPDX-License-Identifier|Copyright\s*(?:\(c\)|©|\d{4})|Licensed under|All rights reserved", re.I)
...
def _in_string(state, raw, spec):
    ...
    for fence in fences:
        ...
        if n % 2:
            state["open"] = None if inside == fence else (fence if inside is None else inside)
            inside = state["open"]
    return was_inside
