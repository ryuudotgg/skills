...
class NoComments(unittest.TestCase):
    ...
    def test_block_comment_body_counts(self):
        out = hook("no_comments.py", write("Write", "/repo/src/a.ts",
                   content="/**\n * Returns the thing.\n */\nexport function f() {}\n"))
        self.assertEqual(len([l for l in out["reason"].splitlines() if l.startswith("  ")]), 3)

    def test_pragmas_shebang_license_pass(self):
        content = ("#!/usr/bin/env python3\n# noqa: E501\n# type: ignore\n"
                   "# SPDX-License-Identifier: MIT\n# Copyright (c) 2026 Ryuu\nx = 1\n")
        self.assertIsNone(hook("no_comments.py", write("Write", "/repo/a.py", content=content)))
        ts = "// eslint-disable-next-line no-console\n// @ts-expect-error bun accepts duplex\nconsole.log(1)\n"
        self.assertIsNone(hook("no_comments.py", write("Write", "/repo/a.ts", content=ts)))

    def test_jsx_and_html_markers(self):
        out = hook("no_comments.py", write("Write", "/repo/a.tsx", content="<div>\n  {/* header */}\n</div>\n"))
        self.assertIn("header", out["reason"])
        out = hook("no_comments.py", write("Write", "/repo/a.html", content="<!-- nav -->\n<nav></nav>\n"))
        self.assertIn("nav", out["reason"])

    def test_prose_and_vendored_skip(self):
        self.assertIsNone(hook("no_comments.py", write("Write", "/repo/README.md", content="# Title\n")))
        self.assertIsNone(hook("no_comments.py", write("Write", "/repo/node_modules/x/a.js", content="// x\n")))

    def test_string_contents_are_not_comments(self):
        ts = "const sample = `\n// not a comment\n# nor this\n`;\nconst x = 1;\n"
        self.assertIsNone(hook("no_comments.py", write("Write", "/repo/a.ts", content=ts)))
        py = 'DOC = """\n# heading in a string\n"""\nx = 1\n'
        self.assertIsNone(hook("no_comments.py", write("Write", "/repo/a.py", content=py)))
        py2 = "x = 1 << 3\n# after a shift\n"
        self.assertIn("after a shift", hook("no_comments.py", write("Write", "/repo/b.py", content=py2))["reason"])
        sh = "cat > f <<'EOF'\n# inside heredoc\nEOF\n# real comment\n"
    ...
    def test_edit_reads_lexical_context_from_disk(self):
        file_text = "const t = `\n// example after\n`;\n/*\n * body line\n */\nconst x = 1;\n"
        out = on_disk("Edit", "a.ts", file_text, old_string="// example before", new_string="// example after")
        self.assertIsNone(out)
        out = on_disk("Edit", "a.ts", file_text, old_string=" * old body", new_string=" * body line")
    ...
    def test_backtick_in_block_comment_does_not_open_string(self):
        ts = "/* uses ` here\n * body */\nconst a = 1;\n// real one\n"
        self.assertIn("real one", hook("no_comments.py", write("Write", "/repo/a.ts", content=ts))["reason"])
...
class ReplyGuard(unittest.TestCase):
    ...
    def test_clean_reply_passes(self):
        self.assertIsNone(self.stop("Done. The hook blocks on dashes and `a -- b` in code is fine.\n\n```\nx \u2014 y\n```"))

    def test_dash_blocks(self):
        self.assertIn("dash", self.stop("The key moved \u2014 one switch drives both.")["reason"])
        self.assertIn("dash", self.stop("The key moved - one switch drives both.")["reason"])
    ...
    def test_only_displayed_findings_are_marked_seen(self):
        put(os.path.join(self.repo, "many.py"), "".join(f"# c{i}\n" for i in range(12)) + "x = 1\n")
    ...
    def test_non_ascii_path_and_rename_are_scanned(self):
        ...
        subprocess.run(GIT + ["commit", "-q", "-m", "old"], cwd=self.repo, check=True)
