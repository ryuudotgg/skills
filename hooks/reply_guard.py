...
def reply_findings(text):
    ...
    if "\u2014" in s or "\u2013" in s or HYPHEN_DASH.search(s):
        out.append("a dash used as punctuation. Use a comma, a colon, parentheses or a full stop.")
    m = OPENERS.search(s)
    if m:
        out.append(f'chatbot filler "{m.group(1).strip()}". Delete the sentence.')
    m = LABEL.search(s)
    if m:
        out.append(f'a bold label with a colon ("{m.group(0)}"). Write it as a sentence or a plain bullet.')
    return out
...
def added_lines(cwd):
    diff = ["git", "-c", "core.quotePath=false", "diff", "--unified=0", "--no-color", "--diff-filter=AMR"]
    ...
    untracked = run(["git", "ls-files", "-z", "--others", "--exclude-standard"], cwd) or ""
...
def tree_findings(cwd, seen):
    hits = []
    root = (run(["git", "rev-parse", "--show-toplevel"], cwd) or "").strip() if cwd else ""
    if not root:
        return hits
...
parts = []
if reply:
    parts.append("Your reply has " + " It also has ".join(reply) + " Rewrite the reply.")
if tree:
